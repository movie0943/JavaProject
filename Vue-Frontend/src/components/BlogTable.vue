...
<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="最後一次修改" width="140" align="left">
      ...
      <el-table-column label="操作" align="left" v-if="showEdit || showDelete">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="handleEdit(scope.$index, scope.row)" v-if="showEdit">修改
          </el-button>
          <el-button
            size="mini"
            @click="handleRestore(scope.$index, scope.row)" v-if="showRestore">還原
          </el-button>
          <el-button
            size="mini"
            type="danger"
            @click="handleDelete(scope.$index, scope.row)" v-if="showDelete">刪除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.articles.length>0 && showDelete"
                 :disabled="this.selItems.length==0" @click="deleteMany">多筆刪除
      </el-button>
      <span></span>
      <el-pagination
        background
  ...
  export default{
    ...
    methods: {
      ...
      loadBlogs(page, count){
        ...
        var url = '';
        if (this.state == -2) {
          url = "/article/article/all" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        } else {
        ...
        getRequest(url).then(resp=> {
          ...
          if (resp.status == 200) {
            ...
            _this.$message({type: 'error', message: '伺服器連接失敗!'});
          }
        }, resp=> {
          _this.loading = false;
        ...
        this.$confirm('將文件還原，是否繼續？','提示',{
          ...
          putRequest('/article/restore', {articleId: row.id}).then(resp=> {
            if (resp.status == 200) {
              ...
              _this.$message({type: 'error', message: '還原失敗!'});
            }
            _this.loading = false;
          });
        }).catch(() => {
      ...
      deleteToDustBin(state){
        var _this = this;
        this.$confirm(state != 2 ? '將該文件刪除，是否繼續?' : '永遠刪除該文件, 是否繼續?', '提示', {
          confirmButtonText: '確定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          ...
          putRequest(url, {aids: _this.dustbinData, state: state}).then(resp=> {
            if (resp.status == 200) {
              ...
              _this.$message({type: 'error', message: '刪除失敗!'});
            }
            _this.loading = false;
            _this.dustbinData = []
          }, resp=> {
