<template>
  <el-container v-loading="loading" class="post-article">
    <el-header class="header">
      <el-select v-model="article.cid" placeholder="選擇所屬分類" style="width: 150px;">
        <el-option
      ...
      <el-input v-model="article.title" placeholder="請輸入標題" style="width: 400px;margin-left: 10px"></el-input>
      <el-tag
        :key="tag"
    ...
    <el-main class="main">
      <div id="editor">
        <mavon-editor style="height: 100%;width: 100%;" ref=md @imgAdd="imgAdd"
                      @imgDel="imgDel" v-model="article.mdContent" language="zh-TW"></mavon-editor>
      </div>
      <div style="display: flex;align-items: center;margin-top: 15px;justify-content: flex-end">
        <el-button @click="cancelEdit" v-if="from!=undefined">放棄修改</el-button>
        <template v-if="from==undefined || from=='draft'">
          <el-button type="primary" @click="saveBlog(1)">發佈</el-button>
        </template>
      ...
      if (from != null && from != '' && from != undefined) {
        ...
        getRequest("/article/" + id).then(resp=> {
          ...
          if (resp.status == 200) {
            ...
            _this.$message({type: 'error', message: '頁面載入失敗!'})
          }
        }, resp=> {
          _this.loading = false;
          _this.$message({type: 'error', message: '頁面載入失敗!'})
    ...
    methods: {
      ...
      saveBlog(state){
        if (!(isNotNullORBlank(this.article.title, this.article.mdContent, this.article.cid))) {
          this.$message({type: 'error', message: '資料不能為空!'});
          return;
        }
        var _this = this;
        _this.loading = true;
        postRequest("/article/", {
          ...
          if (resp.status == 200 && resp.data.status == 'success') {
            ...
            _this.$message({type: 'success', message: state == 0 ? '保存成功!' : '發佈成功!'});
...
//            }
          ...
          _this.loading = false;
          _this.$message({type: 'error', message: state == 0 ? '保存草稿失敗!' : '文章發佈失敗!'});
        })
      },
      imgAdd(pos, $file){
        var _this = this;
